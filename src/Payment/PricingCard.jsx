...
import axios from "../qutree/BaseUrlAPI";
...
const PricingCard = ({ showNav = true, bgBlack = true }) => {
  ...
  useEffect(() => {
    if (selectedPlan) {
      ...
      if (userId) {
        navigate(`/payment?id=${Base64.encode(selectedPlan.id)}&userId=${Base64.encode(userId)}&categoryName=${Base64.encode(selectedTab)}`);
      } else {
